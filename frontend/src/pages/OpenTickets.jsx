...
export default function OpenTickets() {
  const [data, setData] = useState({ SCHN: [], Altitude: [] })
  ...
  const totalOpen = (data.SCHN?.filter(t => t.status_label === 'Open').length || 0) +
                    (data.Altitude?.filter(t => t.status_label === 'Open').length || 0)
  const totalL1   = (data.SCHN?.filter(t => t.status_label === 'Waiting on L1').length || 0) +
                    (data.Altitude?.filter(t => t.status_label === 'Waiting on L1').length || 0)
    ...
    <Layout>
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">
              SCHN &amp; Altitude — Open and Waiting on L1
            </p>
          </div>
        ...
        <div className="flex gap-6 items-start">
          <PlatformColumn name="SCHN" tickets={data.SCHN || []} loading={loading} />
          <PlatformColumn name="Altitude" tickets={data.Altitude || []} loading={loading} />
        </div>
      </div>
    </Layout>
  )
